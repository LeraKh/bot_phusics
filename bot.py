...
def save_user_lead(
    user_id, username, full_name, target_exam, score, segment, phone=None
):
    conn = sqlite3.connect(DB_FILE)
    cur = conn.cursor()
    cur.execute(
...
contact_kb = ReplyKeyboardMarkup(
    keyboard=[
        [KeyboardButton(text="📱 Оставить контакт в 1 клик", request_contact=True)]
    ],
...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="🎯 Готовлюсь к ЕГЭ (10-11 класс)",
                    callback_data="exam_ege",
                )
            ],
            [
                InlineKeyboardButton(
                    text="⚡ Готовлюсь к ОГЭ (8-9 класс)",
                    callback_data="exam_oge",
                )
            ],
        ]
    )
...
async def process_exam_choice(callback: types.CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text=opt[0], callback_data=f"ans_0_{opt[1]}")]
...
async def handle_question(
    ...
    if current_q_idx < len(QUESTIONS) - 1:
        ...
        kb = InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text=opt[0],
                        callback_data=f"ans_{current_q_idx+1}_{opt[1]}",
                    )
                ]
        ...
        await callback.message.answer(
            next_q["text"], reply_markup=kb, parse_mode="Markdown"
        )
...
async def admin_panel(message: types.Message):
    ...
    seg_stat = "\n".join(
        [f"  • {seg if seg else 'Не завершили'}: {cnt}" for seg, cnt in segments]
    )
